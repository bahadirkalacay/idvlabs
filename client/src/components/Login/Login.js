...
const Login = () => {
  ...
  if (userAuth) return <Redirect to={"/products"} />;
  return (
    <>
      <section className="min-h-screen relative py-12 2xl:py-20 overflow-hidden">
        <div className="w-full flex justify-center">
          <div className=" lg:w-1/3 md:w-1/3 sm:w-10/12  px-6 lg:px-12 py-12 lg:py-24  shadow-lg rounded-lg">
            <form onSubmit={formik.handleSubmit}>
              <h3 className="mb-10 text-2xl font-bold font-heading text-blue-400">
              ...
              {loading ? (
                <button
                  disabled
                  className="px-5 py-2.5 w-full bg-gray-500 text-white font-bold rounded-lg transition duration-200"
                >
                  Loading...
                </button>
              ) : (
